...
import { getItemsStockOutlets, deleteItemsStockOutlet, getProducts, getOutlets } from '../../services/api';

export default function ItemsStockOutletsPage() {
    const [itemsStockOutlets, setItemsStockOutlets] = useState([]);
    ...
    const fetchLists = async () => {
        try {
            const [productsResponse, outletsResponse, itemsStockOutletsResponse] = await Promise.all([
                getProducts(),
                getOutlets(),
                getItemsStockOutlets()
            ]);
            ...
            setItemsStockOutlets(itemsStockOutletsResponse.data || []);
        } catch (error) {
    ...
    const init = (page) => {
        getItemsStockOutlets()
            .then(response => {
                ...
                setItemsStockOutlets(response.data.slice(start, end) || []);
    ...
    return (
        <div>
            <h2>Items Stock Outlets</h2>
            <Button onClick={() => setShowTable(!showTable)} className="btn btn-dark mb-2">
                {showTable ? 'Hide Table' : 'Show Table'}
            </Button>
            {showTable && (
                <div className="table-container">
                    <Link to="/items_stock_outlets/add" className="btn btn-dark mb-2">Add Item</Link>
                    <Table striped bordered hover variant="dark">
                        ...
                        <tbody>
                        {Array.isArray(itemsStockOutlets) && itemsStockOutlets.length > 0 && itemsStockOutlets.map(item => (
                            <tr key={item.id}>
                                <td>{item.id}</td>
                                <OverlayTrigger
                                    placement="auto"
                                ...
                                <OverlayTrigger
                                    placement="auto"
                                    delay={{ show: 250, hide: 400 }}
                                    overlay={(props) => renderTooltip(props, outlets[item.outlet_id])}
                                    popperConfig={{
                                        modifiers: [
                                            {
                                                name: 'flip',
                                ...
                                >
                                    <td>{outlets[item.outlet_id]?.name}</td>
                                </OverlayTrigger>
                                <td>{item.quantity}</td>
                                <td>
                                    <Link to={`/items_stock_outlets/edit/${item.id}`} className="btn btn-danger">Edit</Link>
